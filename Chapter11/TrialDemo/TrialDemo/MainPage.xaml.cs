	...
	public sealed partial class MainPage : Page
	{
		...
		void CheckLicense()
		{
			if (_licenseInformation.IsActive)
			{
				...
				{
					if (_licenseInformation.ExpirationDate > DateTime.Now)
					{
						PremiumButton.IsEnabled = true;
					}
				}
			}
			if (_licenseInformation.ProductLicenses["MyCoolNewFeature"].IsActive)
			{
				FeatureButton.IsEnabled = true;
			}
		}

		private async void BuyFeatureButton_Click(object sender, Windows.UI.Xaml.RoutedEventArgs e)
		{
			string myCoolNewFeature = "MyCoolNewFeature";
			if (!_licenseInformation.ProductLicenses[myCoolNewFeature].IsActive)
			{
				try
				{
					//await CurrentAppSimulator.ReloadSimulatorAsync()
					await CurrentAppSimulator.RequestProductPurchaseAsync(myCoolNewFeature);
					CheckLicense();
				}
				catch (Exception ex)
				{
